...
export function UserPanel({ userName, onUserNameChange }: UserPanelProps) {
  const [users, setUsers] = useState<User[]>([
    { id: "1", name: "Sezal", joinedAt: "17:15:00", status: "active", isMuted: false, isAudioMuted: false },
    { id: "2", name: "Alex", joinedAt: "17:16:30", status: "watching", isMuted: false, isAudioMuted: false },
    { id: "3", name: "Jordan", joinedAt: "17:17:45", status: "active", isMuted: true, isAudioMuted: false },
  ])
  ...
  const toggleUserMute = (userId: string) => {
    setUsers(users.map((user) => (user.id === userId ? { ...user, isMuted: !user.isMuted } : user)))
  }

  const toggleUserAudio = (userId: string) => {
    setUsers(users.map((user) => (user.id === userId ? { ...user, isAudioMuted: !user.isAudioMuted } : user)))
  }
  ...
  return (
    <div className="flex flex-col h-full p-4 space-y-4">
      <div>
        <label className="text-sm font-semibold text-foreground block mb-2">My name is:</label>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-semibold text-foreground">
            People{" "}
            <span className="bg-primary text-primary-foreground px-2 py-0.5 rounded-full text-xs">{users.length}</span>
          </span>
        </div>

        <div className="flex-1 overflow-y-auto space-y-2 pr-2">
          {users.map((user) => (
            <Card key={user.id} className="p-3 bg-background border-border hover:border-primary transition-colors">
              <div className="space-y-2">
                {/* User Header */}
                <div className="flex items-center gap-2">
                  <div
                    className={`w-8 h-8 rounded-full ${getAvatarColor(user.name)} text-white flex items-center justify-center text-sm font-semibold relative`}
                  >
                    {user.name[0]}
                    <div
